...
def concept_id(args):
    ...
    all_dates = amr_graph.extract_all_dates(args.tok_file)
    ...
    tok_map = pickle.load(tok_f)
    tok_f.close()

    lemma_f = open(args.lemma_map, 'rb')
    lemma_map = pickle.load(lemma_f)
    lemma_f.close()
    ...
    for (i, (toks, lemmas, poss)) in enumerate(zip(tok_seqs, lemma_seqs, pos_seqs)):
        ...
        unaligned_toks = set(range(n_toks)) - aligned_toks
